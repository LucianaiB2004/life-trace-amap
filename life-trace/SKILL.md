...
6. 按需生成核心故事图：读取 [story-prompt.md](story-prompt.md)，只为最具代表性的事件制作 4:3 横向插图；故事图与完整时间线保持独立，不强制逐事件配图。
...
- 若包含核心故事图，将 PNG 与 HTML 放在同一目录，并在独立故事图模块中按事件映射切换。

...
function renderPortraitGallery(person, events) {
  if (!person.portrait?.src) return '';
  const alt = person.portrait.alt || person.name + '人物像';
  const storyEvents = events.filter((event) => event.storyImage);
  const storySlides = storyEvents.map((event) => (
    '<div class="gallery-slide" data-story-event-id="' + escapeHtml(event.id) + '" data-gallery-title="' + escapeHtml(event.storyTag || event.title) + '" data-gallery-type="' + escapeHtml(event.storyTagType || '故事插图') + '">'
      + '<figure class="story-slide"><img src="' + escapeHtml(event.storyImage) + '" alt="' + escapeHtml(event.storyImageAlt || event.title + '故事插图') + '" width="1536" height="1024" loading="lazy" decoding="async"><figcaption>' + escapeHtml(event.year + ' · ' + event.place + ' · ' + event.title) + '</figcaption></figure>'
      + '</div>'
  )).join('');
  const total = storyEvents.length + 1;
  return '<section class="portrait-gallery" aria-label="人物画廊">'
    + '<div class="module-heading"><div><small id="galleryType">人物主视觉</small><h3 id="galleryTitle">千面玄德</h3></div><div class="gallery-actions"><button type="button" class="gallery-button" data-gallery-direction="-1" aria-label="上一张画面">←</button><button type="button" class="gallery-button" data-gallery-direction="1" aria-label="下一张画面">→</button></div></div>'
    + '<div class="portrait-viewport"><div class="portrait-track"><div class="gallery-slide" data-gallery-title="千面玄德" data-gallery-type="人物主视觉"><figure class="portrait-card"><img src="' + escapeHtml(person.portrait.src) + '" alt="' + escapeHtml(alt) + '" width="1254" height="1254" decoding="async"><figcaption><strong>' + escapeHtml(person.name) + '</strong><span>' + escapeHtml(person.title || '人物像') + '</span></figcaption></figure></div>' + storySlides + '</div></div>'
    + '<div class="gallery-index"><i></i><span id="galleryCounter">01 / ' + String(total).padStart(2, '0') + ' · 可左右滑动</span></div>'
    + '</section>';
}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html,body{margin:0;min-height:100%;background:#030913;color:var(--paper);font-family:"Microsoft YaHei","PingFang SC",system-ui,sans-serif}button,select{font:inherit}
    ...
    .gallery-slide{flex:0 0 100%;min-width:100%;aspect-ratio:4/3;scroll-snap-align:start}.gallery-slide>figure{width:100%;height:100%}.portrait-card{margin:0;min-width:0;min-height:0;border:0;background:radial-gradient(circle at 50% 36%,#f2e2c3 0%,#d9b17c 70%,#84503a 100%);box-shadow:none}.portrait-card img{width:100%;height:100%;max-height:none;object-fit:contain}.portrait-card figcaption{position:absolute;left:10px;right:10px;bottom:9px;z-index:2;display:flex;align-items:end;justify-content:space-between;padding:22px 10px 9px;background:linear-gradient(transparent,rgba(42,24,17,.82));color:#fff8eb}.portrait-card figcaption strong{font:700 20px "STKaiti","KaiTi",serif}.portrait-card figcaption span{font-size:9px;color:#dec8a5}.gallery-index{display:flex;align-items:center;gap:9px;margin-top:10px;color:#958b7e;font-size:9px}.gallery-index i{width:34px;height:2px;background:#a92f24}
    .story-slide{position:relative;margin:0;overflow:hidden;background:#0f1112}.story-slide img{display:block;width:100%;height:100%;object-fit:contain}.story-slide figcaption{position:absolute;left:0;right:0;bottom:0;padding:36px 12px 11px;background:linear-gradient(transparent,rgba(15,17,18,.9));color:#f2e7d5;font-size:10px;line-height:1.5}
    ...
    .shell[data-theme="light"] aside{background:#efe9dd;color:#2a241e;scrollbar-color:#b6a58e transparent}.shell[data-theme="light"] .side-head{background:linear-gradient(to bottom,#f7f2e8 82%,rgba(247,242,232,.94));border-color:rgba(89,64,38,.18)}.shell[data-theme="light"] .summary{color:#6d6358}.shell[data-theme="light"] .portrait-gallery{border-color:rgba(89,64,38,.18);background:#faf6ed;box-shadow:0 12px 28px rgba(64,46,28,.08)}.shell[data-theme="light"] .module-heading h3,.shell[data-theme="light"] .timeline-heading{color:#2a241e}.shell[data-theme="light"] .gallery-button{border-color:rgba(89,64,38,.22);color:#6c5537}.shell[data-theme="light"] .story-slide{background:#e3d9c8}.shell[data-theme="light"] .timeline-card{border-color:rgba(89,64,38,.14);background:#faf7f0;color:#2a241e;box-shadow:0 7px 22px rgba(64,46,28,.05)}.shell[data-theme="light"] .timeline-card.active{border-color:#a92f24;box-shadow:0 12px 30px rgba(64,46,28,.11)}.shell[data-theme="light"] .timeline-card p{color:#655d54}.shell[data-theme="light"] .filter{border-color:rgba(89,64,38,.2);color:#685b4c}.shell[data-theme="light"] .filter.active{border-color:#a92f24;background:#a92f24;color:#fff8eb}.map-stage[data-map-theme="light"] .play-panel{border-color:rgba(91,65,36,.2);background:rgba(255,252,246,.92);color:#2a241e}.map-stage[data-map-theme="light"] .reset{border-color:rgba(91,65,36,.22);color:#6b563b}.map-stage[data-map-theme="light"] .route-progress{background:rgba(91,65,36,.14)}
    @media(max-width:1050px){.shell{grid-template-columns:minmax(0,1.75fr) minmax(350px,1fr)}.amap-badge{display:none}.stats{display:none}}
    @media(max-width:900px){.shell{display:block;height:auto;overflow:visible}.map-stage{height:72vh;min-height:560px;border-right:0;border-bottom:1px solid var(--line)}aside{height:auto;overflow:visible}.side-head{position:relative}.map-stage header{padding:18px}.map-status{top:102px;left:18px}.play-panel{bottom:16px;width:calc(100% - 32px)}.timeline{max-width:720px;margin:auto}}
    @media(max-width:560px){.map-stage{height:76vh;min-height:590px}.map-stage header>div:first-child{max-width:calc(100vw - 116px)}.map-stage .title{font-size:34px}.map-stage .subtitle{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.theme-button{min-width:44px;padding:6px 8px}.map-status{top:89px;max-width:calc(100% - 36px)}.play-actions{display:grid;grid-template-columns:1fr auto auto}.play{min-width:0}.now-year{grid-column:1/-1;text-align:left;font-size:15px}.timeline{padding:14px 12px 40px}.side-head{padding:21px 16px}.portrait-gallery{padding:11px}.portrait-card{min-height:225px}.portrait-card img{max-height:235px}.timeline-card{padding:17px 15px 16px 19px}.timeline-card.has-visual:after{display:none}}
...
<body>
  <main class="shell" data-theme="light">
    ...
    <aside>
      ...
      <div class="timeline">${renderPortraitGallery(data.person, data.events)}<div class="timeline-heading"><span>人生纪事</span><b>${data.events.length} 个节点</b></div>${renderCards(data.events)}</div>
  ...
  <script>
    ...
    let map,infoWindow,markers=[],segments=[],current=0,playing=false,timer=null,phase='全部阶段',theme=readStoredTheme(),galleryIndex=0,galleryFrame=null,galleryLoopTimer=null;
    ...
    function setupGalleryLoop(){if(!portraitTrack||gallerySlides.length<2)return;const firstClone=gallerySlides[0].cloneNode(true);const lastClone=gallerySlides[gallerySlides.length-1].cloneNode(true);firstClone.dataset.galleryClone='first';lastClone.dataset.galleryClone='last';firstClone.setAttribute('aria-hidden','true');lastClone.setAttribute('aria-hidden','true');portraitTrack.prepend(lastClone);portraitTrack.append(firstClone)}
    function normalizeGalleryLoop(){if(!portraitTrack||gallerySlides.length<2)return;const width=Math.max(1,portraitTrack.clientWidth);const physicalIndex=Math.round(portraitTrack.scrollLeft/width);if(physicalIndex===0)portraitTrack.scrollTo({left:gallerySlides.length*width,behavior:'auto'});else if(physicalIndex===gallerySlides.length+1)portraitTrack.scrollTo({left:width,behavior:'auto'})}
    function updateGalleryMeta(index){const slide=gallerySlides[index];if(!slide)return;galleryIndex=index;galleryTitle.textContent=slide.dataset.galleryTitle||'人物与故事';galleryType.textContent=slide.dataset.galleryType||'核心故事图';galleryCounter.textContent=String(index+1).padStart(2,'0')+' / '+String(gallerySlides.length).padStart(2,'0')+' · 可左右滑动'}
    function showGallerySlide(index,smooth=true){if(!portraitTrack||!gallerySlides.length)return;const total=gallerySlides.length;const target=(index+total)%total;const forwardWrap=smooth&&galleryIndex===total-1&&index>=total;const backwardWrap=smooth&&galleryIndex===0&&index<0;const physicalTarget=forwardWrap?total+1:backwardWrap?0:target+1;portraitTrack.scrollTo({left:physicalTarget*portraitTrack.clientWidth,behavior:smooth?'smooth':'auto'});updateGalleryMeta(target)}
    function syncGalleryFromEvent(item){if(!item.storyImage)return;const index=gallerySlides.findIndex((slide)=>slide.dataset.storyEventId===item.id);if(index>=0)showGallerySlide(index,true)}
    ...
    function activate(index,scroll,focus){current=index;cards.forEach((card,i)=>card.classList.toggle('active',i===index));nowYear.textContent=DATA.events[index].year;updateProgress();syncGalleryFromEvent(DATA.events[index]);syncMapState(focus);if(scroll)cards[index].scrollIntoView({behavior:'smooth',block:'center'})}
    function stop(){playing=false;clearTimeout(timer);playButton.textContent='▶ 播放足迹'}
    function resetPlayback(){stop();activate(0,true,false);showGallerySlide(0,true);phase='全部阶段';document.querySelectorAll('.filter').forEach((button)=>button.classList.toggle('active',button.dataset.phase==='全部阶段'));cards.forEach((card)=>card.classList.remove('hidden'));syncMapState(false);if(infoWindow)infoWindow.close();if(map&&markers.length)map.setFitView(markers,false,[120,80,125,80],5)}
    ...
    document.querySelectorAll('[data-gallery-direction]').forEach((button)=>button.addEventListener('click',()=>showGallerySlide(galleryIndex+Number(button.dataset.galleryDirection),true)));
    if(portraitTrack)portraitTrack.addEventListener('scroll',()=>{cancelAnimationFrame(galleryFrame);clearTimeout(galleryLoopTimer);galleryFrame=requestAnimationFrame(()=>{const physicalIndex=Math.round(portraitTrack.scrollLeft/Math.max(1,portraitTrack.clientWidth));const index=physicalIndex===0?gallerySlides.length-1:physicalIndex===gallerySlides.length+1?0:physicalIndex-1;if(index>=0&&index<gallerySlides.length)updateGalleryMeta(index)});galleryLoopTimer=setTimeout(normalizeGalleryLoop,160)});

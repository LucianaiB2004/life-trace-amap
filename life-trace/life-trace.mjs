...
<body>
  ...
  <script>
    ...
    let map,infoWindow,markers=[],segments=[],current=0,playing=false,timer=null,phase='全部阶段',theme=readStoredTheme(),galleryIndex=0,galleryFrame=null;
    ...
    function showGallerySlide(index,smooth=true){if(!portraitTrack||!gallerySlides.length)return;const target=(index+gallerySlides.length)%gallerySlides.length;portraitTrack.scrollTo({left:target*portraitTrack.clientWidth,behavior:smooth?'smooth':'auto'});updateGalleryMeta(target)}
    ...
    if(portraitTrack)portraitTrack.addEventListener('scroll',()=>{cancelAnimationFrame(galleryFrame);galleryFrame=requestAnimationFrame(()=>{const index=Math.round(portraitTrack.scrollLeft/Math.max(1,portraitTrack.clientWidth));if(index>=0&&index<gallerySlides.length)updateGalleryMeta(index)})});
